...
function diaLabel(value: string) {
  return DIA_LABELS[value] ?? value;
}
...
function ModoReuniaoPage() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
            ...
            <SelectContent>
              {(pautas ?? []).map((p) => (
                <SelectItem key={p.id} value={p.id}>
                  {diaLabel(p.dia_semana)} — {p.titulo}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Button
            ...
            className="gap-2"
            title={!blocos || blocos.length === 0 ? "Adicione ao menos um bloco a esta pauta para poder apresentar" : undefined}
          >
      ...
      {selected && blocos && blocos.length === 0 && (
        <div className="rounded-lg border border-warning/30 bg-warning/5 px-4 py-3 text-sm text-warning-foreground">
          Esta pauta ainda não tem blocos, por isso "Iniciar apresentação" está desabilitado.{" "}
          <Link to="/pautas" className="underline font-medium">
            Adicione blocos na página de Pautas
          </Link>{" "}
          para poder apresentar em tela cheia.
        </div>
      )}
